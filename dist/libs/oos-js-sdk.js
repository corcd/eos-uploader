...
_xamzrequire = function () {
            ...
            OOS.Signers.V4 = inherit(OOS.Signers.RequestSigner, {
                ...
                canonicalString: function canonicalString() {
                    ...
                    if (this.serviceName !== "s3")
                        pathname = OOS.util.uriEscapePath(pathname);
            ...
            OOS.ParamValidator = OOS.util.inherit({
                    ...
                    for (var i = 0; i < acceptedTypes.length; i++) {
                        if (typeof acceptedTypes[i] === "string") {
                            ...
                            if (value instanceof acceptedTypes[i])
                                return true;
                            if (OOS.util.isType(value, acceptedTypes[i]))
                                return true;
                ...
                validatePayload: function validatePayload(value, context) {
                    ...
                    if (OOS.util.isNode()) {
                        ...
                        if (OOS.util.Buffer.isBuffer(value) || value instanceof Stream)
                            return;
                    }
                    var types = ["Buffer", "Stream", "File", "Blob", "ArrayBuffer", "DataView"];
                    if (value) {
                        for (var i = 0; i < types.length; i++) {
                            if (OOS.util.isType(value, types[i]))
                                return;
                            if (OOS.util.typeName(value.constructor) === types[i])
                                return;
                        }
                    }
            ...
            OOS.EventListeners = {
                Core: new SequentialExecutor().addNamedListeners(function (add, addAsync) {
                    ...
                    addAsync("COMPUTE_SHA256", "afterBuild", function COMPUTE_SHA256(req, done) {
                        ...
                        if (!req.service.api.signatureVersion && !authtype)
                            return done();
                    ...
                    addAsync("SIGN", "sign", function SIGN(req, done) {
                        ...
                        var authtype = operation ? operation.authtype : "";
                        if (!service.api.signatureVersion && !authtype)
                            return done();
    ...
    61: [function (require, module, exports) {
            ...
            function translate(value, shape) {
                if (!shape || value === undefined || value === null)
                    return undefined;
                switch (shape.type) {
                    case "structure":
                        return translateStructure(value, shape);
            ...
            (function (process, setImmediate) {
                ...
                var util = {
                    ...
                    applyClockOffset: function applyClockOffset(serverTime) {
                        if (serverTime)
                            OOS.config.systemClockOffset = serverTime - new Date().getTime();
                    },
    ...
    37: [function (require, module, exports) {
            ...
            OOS.Config = OOS.util.inherit({
                ...
                setPromisesDependency: function setPromisesDependency(dep) {
                    ...
                    if (OOS.S3 && OOS.S3.ManagedUpload)
                        constructors.push(OOS.S3.ManagedUpload);
...
OOS.apiLoader.services["s3"]["2006-03-01"] = {
    version: "2.0",
    metadata: {
            ...
            OOS.util.update(OOS.STS.prototype, {
                credentialsFrom: function credentialsFrom(data, credentials) {
                    if (!data)
                        return null;
                    if (!credentials)
                        credentials = new OOS.TemporaryCredentials();
                    credentials.expired = false;
                    credentials.accessKeyId = data.Credentials.AccessKeyId;
...
OOS.apiLoader.services["sts"]["2011-06-15"] = {
    version: "2.0",
    metadata: {
