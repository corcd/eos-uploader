...
var wrapEventCallback = function (eventCallback) {
    ...
    return function (t, eventParam, result) {
        ...
        if (result.CommonMsg.Status > 300) {
            return eventCallback(t, eventParam, new Error('status:' +
...
var abortRequest = function (uploadCheckpoint, funcName, that) {
    if (uploadCheckpoint && uploadCheckpoint.uploadId) {
        that.abortMultipartUpload({
            ...
            if (err) {
                that.log.runLog('warn', funcName, 'abort multipart upload failed, bucket:' +
            ...
            that.log.runLog('warn', funcName, 'abort multipart upload succeed, bucket:' +
                uploadCheckpoint.bucket +
                ', key:' +
                uploadCheckpoint.key +
                ', uploadId:' +
        ...
        return function () {
            ...
            var doUploadPart = function (contentMd5) {
                ...
                ctx.that.uploadPart(uploadPartParam, function (err, result) {
                    ...
                    if (err) {
                    ...
                    else if (result.CommonMsg.Status >= 500 ||
                        ...
                        ctx.that.log.runLog('debug', ctx.funcName, 'Part ' +
    ...
    ObsClient.prototype.uploadFile = function (param, callback) {
        ...
        if (!uploadCheckpoint.uploadId) {
            ...
            if (!contentType && uploadCheckpoint.key) {
                contentType =
                    that.util.mimeTypes[uploadCheckpoint.key.substring(uploadCheckpoint.key.lastIndexOf('.') + 1)];
            }
            if (!contentType && uploadCheckpoint.sourceFile.name) {
                contentType =
                    that.util.mimeTypes[uploadCheckpoint.sourceFile.name.substring(uploadCheckpoint.sourceFile.name.lastIndexOf('.') + 1)];
            }
