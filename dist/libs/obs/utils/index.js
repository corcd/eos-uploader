...
Utils.prototype.doNegotiation = function (funcName, param, callback, checkBucket, checkCache, setCache) {
    ...
    this.doExec(negotiateMethod, checkBucket ? { Bucket: param.Bucket } : {}, function (err, result) {
        ...
        if ((checkBucket && result.CommonMsg.Status === 404) ||
                    ...
                    if (!isArray(obj[sentAs])) {
                        arr[0] =
                            model[key].items.type === 'object'
                                ? this.jsonToObject(model[key].items.parameters, obj[sentAs], null)
                                : obj[sentAs]['#text'] || '';
                    }
                    else {
                        for (let i = 0; i < obj[sentAs].length; i++) {
                            arr[i] =
                                model[key].items.type === 'object'
                                    ? this.jsonToObject(model[key].items.parameters, obj[sentAs][i], null)
                                    : obj[sentAs][i]['#text'];
                        }
                    }
                    opt[key] = arr;
...
Utils.prototype.makeParam = function (methodName, param) {
    ...
    var model = signatureContext.signature === 'obs'
        ? obsModel[methodName]
        : v2Model[methodName];
    ...
    if ('srcFile' in opt) {
        if (opt.srcFile instanceof window.File ||
...
Utils.prototype.getRequest = function (methodName, serverback, signatureContext, retryCount, params, bc) {
    ...
    log.runLog('debug', methodName, 'response msg :' +
        'statusCode:' +
        serverback.status +
        ', headers:' +
        headersStr);
    ...
    return makeObjFromXml(body, function (err, re) {
        if (err) {
            ...
            log.runLog('error', methodName, 'request error with error code:' +
                opt.CommonMsg.Code +
                ', error message:' +
                opt.CommonMsg.Message +
                ', request id:' +
...
Utils.prototype.doAuth = function (opt, methodName, signatureContext) {
    ...
    for (let i = 0; i < temp.length; i++) {
        let key = temp[i].key;
        let val = key.indexOf(signatureContext.headerMetaPrefix) === 0
            ? temp[i].value.trim()
            : temp[i].value;
...
Utils.prototype.createV2SignedUrlSync = function (param) {
    ...
    if (signatureContext.signature.toLowerCase() === 'obs' &&
        specialParam === 'storagePolicy') {
        specialParam = 'storageClass';
    }
    else if (signatureContext.signature.toLowerCase() === 'v2' &&
        specialParam === 'storageClass') {
        specialParam = 'storagePolicy';
    }
    ...
    for (let key in interestHeaders) {
        if (key.length > signatureContext.headerPrefix.length &&
            key.slice(0, signatureContext.headerPrefix.length) ===
                signatureContext.headerPrefix) {
            temp[i++] = key;
        }
    ...
    return {
        ...
        SignedUrl: (this.isSecure ? 'https' : 'http') +
            '://' +
            host +
            ':' +
            this.port +
...
Utils.prototype.createPostSignatureSync = function (param) {
    ...
    for (let key in formParams) {
        ...
        if (allowedResponseHttpHeaderMetadataNames.indexOf(key) < 0 &&
