...
ObsClient.prototype.putObject = function (param, callback) {
  ...
  if (!('ContentType' in param)) {
    if ('Key' in param) {
      param.ContentType = this.util.mimeTypes[
        param.Key.substring(param.Key.lastIndexOf('.') + 1)
      ]
    }
...
ObsClient.prototype.appendObject = function (param, callback) {
  ...
  if (!('ContentType' in param)) {
    ...
    if (!param.ContentType && 'SourceFile' in param) {
      param.ContentType = this.util.mimeTypes[
        param.SourceFile.substring(param.SourceFile.lastIndexOf('.') + 1)
      ]
    }
  }
...
ObsClient.prototype.copyObject = function (param, callback) {
  ...
  if (key in param) {
    ...
    if (index > 0) {
      param[key] =
        this.util.encodeURIWithSafe(val.slice(0, index)) + val.slice(index)
    } else {
...
ObsClient.prototype.restoreObject = function (param, callback) {
  this.exec('RestoreObject', param, function (err, result) {
    if (!err && result.InterfaceResult && result.CommonMsg.Status < 300) {
      result.InterfaceResult.RestoreStatus =
        result.CommonMsg.Status === 200 ? 'AVALIABLE' : 'INPROGRESS'
    }
    callback(err, result)
  })
}
...
ObsClient.prototype.initLog = function (param) {
  ...
  msg.push(
    'Access Mode=' + (this.util.path_style ? 'Path' : 'Virtual Hosting') + ']'
  )
...
ObsClient.prototype.factory = function (param) {
  ...
  this.util.initFactory(
    ...
    param.is_signature_negotiation,
    param.is_cname,
    param.url_prefix,
    param.region_domains,
    param.setRequestHeaderHook,
    param.useRawXhr
  )
}

ObsClient.prototype.refresh = function (
  access_key_id,
  secret_access_key,
  security_token
) {
