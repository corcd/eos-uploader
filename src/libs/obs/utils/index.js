...
function createV4Signature(shortDate, sk, region, stringToSign) {
  var dateKey = crypto
    .createHmac('sha256', 'AWS4' + sk)
    .update(shortDate)
    .digest()
  var dateRegionKey = crypto
    .createHmac('sha256', dateKey)
    .update(region)
    .digest()
  var dateRegionServiceKey = crypto
    .createHmac('sha256', dateRegionKey)
    .update('s3')
    .digest()
  var signingKey = crypto
...
Utils.prototype.initFactory = function (
  ak,
  sk,
  isSecure,
  server,
  ...
  this.signatureContext =
    signature === 'obs' ? obsSignatureContext : v2SignatureContext
...
Utils.prototype.doNegotiation = function (
  funcName,
  param,
  callback,
  checkBucket,
  ...
  if (checkCache && param.Bucket) {
    ...
    if (
      item &&
      item.signatureContext &&
      item.expireTime > new Date().getTime()
    ) {
  ...
  this.doExec(
    negotiateMethod,
    checkBucket ? { Bucket: param.Bucket } : {},
    function (err, result) {
      if (err) {
        callback(err, null)
        if (o) {
      ...
      if (
        (checkBucket && result.CommonMsg.Status === 404) ||
        result.CommonMsg.Status >= 500
      ) {
        callback(err, result)
        if (o) {
          o.s = 0
      ...
      if (
        ...
        result.InterfaceResult.ApiVersion >= '3.0'
      ) {
      ...
      if (setCache) {
        that.bucketSignatureCache[param.Bucket] = {
          ...
          expireTime:
            new Date().getTime() +
            15 +
            Math.ceil(Math.random() * 5) * 60 * 1000,
        }
      }
...
Utils.prototype.toXml = function (
  mXml,
  xmlMeta,
  root,
  sentAs,
...
Utils.prototype.buildXml = function (
  mXml,
  xmlMeta,
  key,
  sentAs,
  ...
  if (type === 'array') {
    for (let i = 0; i < mXml[key].length; i++) {
      if (xmlMeta.items.type === 'object') {
        let result = this.toXml(
          mXml[key][i],
          xmlMeta.items.parameters,
          null,
          null,
          signatureContext
        )
        ...
        xml +=
          ...
          sentAs +
          '>' +
          String(this[key + 'Adapter'](mXml[key][i], signatureContext))
            .replace(/&/g, '&amp;')
            .replace(/'/g, '&apos;')
            .replace(/"/g, '&quot;') +
          '</' +
    ...
    xml +=
      ...
      sentAs +
      '>' +
      String(this[key + 'Adapter'](mXml[key], signatureContext))
        .replace(/&/g, '&amp;')
        .replace(/'/g, '&apos;')
        .replace(/"/g, '&quot;') +
      '</' +
...
Utils.prototype.buildObject = function (model, obj, key, opt) {
      ...
      if (sentAs in obj) {
        if (model[key].type === 'object') {
          ...
          if (!isArray(obj[sentAs])) {
            arr[0] =
              model[key].items.type === 'object'
                ? this.jsonToObject(
                    model[key].items.parameters,
                    obj[sentAs],
                    null
                  )
                : obj[sentAs]['#text'] || ''
  ...
  if (opt[key] === undefined) {
    if (model[key].type === 'object') {
      opt[key] = model[key].parameters
        ? this.jsonToObject(model[key].parameters, null, null)
        : {}
    } else if (model[key].type === 'array') {
      opt[key] = []
    } else {
...
Utils.prototype.makeParam = function (methodName, param) {
  ...
  var model =
    signatureContext.signature === 'obs'
      ? obsModel[methodName]
      : v2Model[methodName]
  ...
  for (let key in model.parameters) {
    ...
    if (
      ...
      (_value === null ||
        _value === undefined ||
        (Object.prototype.toString.call(_value) === '[object String]' &&
          _value === ''))
    ) {
    ...
    if (_value !== null && _value !== undefined) {
      ...
      if (meta.location === 'uri') {
        ...
        if (meta.type === 'object') {
          if (signatureContext.headerMetaPrefix === sentAs) {
            for (let item in _value) {
              ...
              exheaders[
                item.indexOf(sentAs) === 0 ? item : sentAs + item
              ] = encodeURIWithSafe(value, safe)
            }
          }
          ...
          exheaders[sentAs] = encodeURIWithSafe(
            String(_value),
            safe,
            meta.skipEncoding
          )
        ...
        if (
          meta.type !== 'number' ||
          (meta.type === 'number' && Number(value) >= 0)
        ) {
          urlPath +=
            sep +
            encodeURIWithSafe(sentAs, '/') +
            '=' +
            encodeURIWithSafe(String(value), '/')
        }
  ...
  exheaders.Host =
    this.server + (this.port === 80 || this.port === 443 ? '' : ':' + this.port)
  ...
  if ('srcFile' in opt) {
    if (
      opt.srcFile instanceof window.File ||
      opt.srcFile instanceof window.Blob
    ) {
      let fileSize = opt.srcFile.size
      if (
        'Content-Length' in opt.headers ||
        'PartSize' in opt ||
        'Offset' in opt
      ) {
        ...
        partSize =
          partSize && partSize > 0 && partSize <= fileSize - offset
            ? partSize
            : fileSize - offset
...
Utils.prototype.parseCommonHeaders = function (opt, headers, signatureContext) {
  ...
  opt.InterfaceResult.RequestId =
    headers[signatureContext.headerPrefix + 'request-id']
...
Utils.prototype.contrustCommonMsg = function (
  opt,
  obj,
  headers,
  signatureContext
...
Utils.prototype.getRequest = function (
  methodName,
  serverback,
  signatureContext,
  retryCount,
  ...
  var model =
    signatureContext.signature === 'obs'
      ? obsModel[methodName + 'Output']
      : v2Model[methodName + 'Output']
  model = model || {}
  ...
  log.runLog(
    'info',
    methodName,
    'get response start, statusCode:' + serverback.status
  )
  log.runLog(
    'debug',
    methodName,
  ...
  var doLog = function () {
    ...
    log.runLog(
      'debug',
      methodName,
      'exec interface ' + methodName + ' finish, ' + logMsg
    )
    bc(null, opt)
  }

  if (
    serverback.status >= 300 &&
    serverback.status < 400 &&
    serverback.status !== 304 &&
    retryCount <= 5
  ) {
    ...
    let bucketLocation =
      headers['x-amz-bucket-region'] || headers['x-obs-bucket-location']
    if (bucketLocation && regionDomains[bucketLocation]) {
      let regionServer =
        (this.isSecure ? 'https://' : 'http://') + regionDomains[bucketLocation]
      if (isFunction(this.setRequestHeaderHook)) {
        this.setRequestHeaderHook(
          headers,
          params,
          methodName,
          regionDomains[bucketLocation]
        )
      }
        ...
        return makeObjFromXml(body, function (err, result) {
          if (err) {
            log.runLog(
              'error',
              methodName,
              'change xml to json err [' + headerTostring(err) + ']'
            )
            return bc(err, null)
          }
          ...
          if (isObject(tempResult)) {
            for (let key in obj) {
              if (obj[key].location === 'xml') {
                opt.InterfaceResult[key] = that.jsonToObject(
                  obj,
                  tempResult,
                  key
                )[key]
  ...
  log.runLog(
    'info',
    methodName,
    'request finished with request id:' + opt.CommonMsg.RequestId
  )
  log.runLog('debug', methodName, respMsg)
...
Utils.prototype.makeRequest = function (methodName, opt, retryCount, bc) {
  ...
  opt.headers[signatureContext.headerPrefix + 'date'] = isV4
    ? getDates(utcDateStr)[1]
    : utcDateStr
  ...
  if (this.ak && this.sk && methodName !== negotiateMethod) {
    if (this.securityToken) {
      opt.headers[
        signatureContext.headerPrefix + 'security-token'
      ] = this.securityToken
    }
    if (isV4) {
  ...
  var _isSecure = opt.protocol
    ? opt.protocol.toLowerCase().indexOf('https') === 0
    : this.isSecure
  ...
  if (
    opt.dstFile &&
    opt.dstFile !== 'file' &&
    (opt.dstFile === 'arraybuffer' || opt.dstFile === 'blob')
  ) {
  ...
  if (!this.useRawXhr) {
    ...
    var baseUrl = _isSecure
      ? 'https://' + this.urlPrefix + host + portInfo
      : 'http://' + this.urlPrefix + host + portInfo
    var reopt = {
      method: method,
    ...
    if (opt.srcFile) {
      ...
      try {
        if (opt.Offset >= 0 && opt.PartSize > 0) {
          srcFile = this.sliceBlob(
            srcFile,
            opt.Offset,
            opt.Offset + opt.PartSize
          )
      ...
      axios
        ...
        .then(function (response) {
          log.runLog(
            'info',
            methodName,
            'http cost ' + (new Date().getTime() - start) + ' ms'
          )
          that.getRequest(
            methodName,
            response,
        ...
        .catch(function (err) {
          ...
          log.runLog(
            'error',
            methodName,
            'Send request to service error [' + headerStr + ']'
          )
          log.runLog(
            'info',
            methodName,
  ...
  xhr.open(
    method,
    (_isSecure
      ? 'https://' + this.urlPrefix + host
      : 'http://' + this.urlPrefix + host) + path
  )
...
Utils.prototype.sendRequest = function (funcName, opt, backcall, retryCount) {
  ...
  that.makeRequest(funcName, opt, retryCount, function (err, msg) {
    if (err && err.message === 'redirect') {
      ...
      opt.port =
        uri.port ||
        (opt.protocol && opt.protocol.toLowerCase().indexOf('https') === 0
          ? 443
          : 80)
...
Utils.prototype.doAuth = function (opt, methodName, signatureContext) {
  ...
  for (let i = 0; i < temp.length; i++) {
    let key = temp[i].key
    let val =
      key.indexOf(signatureContext.headerMetaPrefix) === 0
        ? temp[i].value.trim()
        : temp[i].value
  ...
  opt.headers.Authorization =
    signatureContext.authPrefix +
    ' ' +
    this.ak +
    ':' +
    crypto.createHmac('sha1', this.sk).update(stringToSign).digest('base64')
}
...
Utils.prototype.v4Auth = function (opt, methodName, signatureContext) {
  ...
  var credenttial =
    this.ak + '/' + shortDate + '/' + this.region + '/s3/aws4_request'
...
Utils.prototype.createSignedUrlSync = function (param) {
  var signatureContext = param.signatureContext || this.signatureContext
  return signatureContext.signature.toLowerCase() === 'v4'
    ? this.createV4SignedUrlSync(param)
    : this.createV2SignedUrlSync(param)
}
...
Utils.prototype.createV2SignedUrlSync = function (param) {
  ...
  if (
    signatureContext.signature.toLowerCase() === 'obs' &&
    specialParam === 'storagePolicy'
  ) {
    specialParam = 'storageClass'
  } else if (
    signatureContext.signature.toLowerCase() === 'v2' &&
    specialParam === 'storageClass'
  ) {
    specialParam = 'storagePolicy'
  }
  ...
  if (
    param.Headers &&
    param.Headers instanceof Object &&
    !(param.Headers instanceof Array)
  ) {
  ...
  if (
    param.QueryParams &&
    param.QueryParams instanceof Object &&
    !(param.QueryParams instanceof Array)
  ) {
  ...
  if (
    this.securityToken &&
    !queryParams[signatureContext.headerPrefix + 'security-token']
  ) {
    queryParams[
      signatureContext.headerPrefix + 'security-token'
    ] = this.securityToken
  ...
  for (let name in headers) {
    ...
    if (
      key === 'content-type' ||
      key === 'content-md5' ||
      (key.length > signatureContext.headerPrefix.length &&
        key.slice(0, signatureContext.headerPrefix.length) ===
          signatureContext.headerPrefix)
    ) {
  ...
  for (let i = 0; i < queryParamsKeys.length; i++) {
    ...
    if (
      allowedResourceParameterNames.indexOf(key.toLowerCase()) >= 0 ||
      key.toLowerCase().indexOf(signatureContext.headerPrefix) === 0
    ) {
      flag = true
...
Utils.prototype.createV4SignedUrlSync = function (param) {
  ...
  headers.Host =
    host + (this.port === 80 || this.port === 443 ? '' : ':' + this.port)
  ...
  for (let i = 0; i < queryParamsKeys.length; ) {
    canonicalQueryString +=
      queryParamsKeys[i] + '=' + _queryParams[queryParamsKeys[i]]
...
Utils.prototype.createPostSignatureSync = function (param) {
  ...
  if (
    param.FormParams &&
    param.FormParams instanceof Object &&
    !(param.FormParams instanceof Array)
  ) {
  ...
  if (
    this.securityToken &&
    !formParams[signatureContext.headerPrefix + 'security-token']
  ) {
    formParams[
      signatureContext.headerPrefix + 'security-token'
    ] = this.securityToken
  ...
  for (let key in formParams) {
    ...
    if (
      allowedResponseHttpHeaderMetadataNames.indexOf(key) < 0 &&
      conditionAllowKeys.indexOf(key) < 0 &&
      key.indexOf(signatureContext.headerPrefix) !== 0
    ) {
      continue
    }
