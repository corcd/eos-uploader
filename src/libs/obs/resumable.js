...
var wrapCallback = function (callback, log, methodName) {
  ...
  return function (err, result) {
    log.runLog(
      'info',
      methodName,
      'ObsClient cost ' + (new Date().getTime() - start) + ' ms'
    )
        ...
        if (err) {
          that.log.runLog(
            ...
            'abort multipart upload failed, bucket:' +
              uploadCheckpoint.bucket +
              ', key:' +
              uploadCheckpoint.key +
              ', uploadId:' +
  ...
  var createUploadPartTask = function (part) {
    return function () {
        ...
        ctx.that.uploadPart(uploadPartParam, function (err, result) {
          ...
          if (err) {
            ...
            (result.CommonMsg.Status === 400 &&
              result.CommonMsg.Code === 'BadDigest')
          ) {
            ...
            ctx.uploadCheckpoint.md5 = calculateUploadCheckpointMD5(
              ctx.uploadCheckpoint
            )
      ...
      if (
        ctx.verifyMd5 &&
        window.FileReader &&
        (ctx.uploadCheckpoint.sourceFile instanceof window.File ||
          ctx.uploadCheckpoint.sourceFile instanceof window.Blob)
      ) {
        let _sourceFile = sliceBlob(
          ctx.uploadCheckpoint.sourceFile,
          part.offset,
          part.offset + part.partSize
        )
        ...
        fr.onerror = function (ex) {
          ctx.that.log.runLog(
            'error',
            ctx.funcName,
            'Caculate md5 for part ' + String(part.partNumber) + ' failed'
          )
          doUploadPart()
        }
...
resumable.extend = function (ObsClient) {
  ObsClient.prototype.uploadFile = function (param, callback) {
    ...
    if (
      param.UploadCheckpoint &&
      param.UploadCheckpoint.sourceFile &&
      param.UploadCheckpoint.fileStat &&
      param.UploadCheckpoint.uploadId &&
      param.UploadCheckpoint.md5 ===
        calculateUploadCheckpointMD5(param.UploadCheckpoint)
    ) {
    ...
    if (!uploadCheckpoint) {
      ...
      if (
        !(sourceFile instanceof window.File) &&
      ...
      that.log.runLog(
        'debug',
        funcName,
        'Begin to uploadFile to OBS from file:' + sourceFile.name
      )
      ...
      if (fileSize === 0) {
        ...
        if (partSize > maxPartSize) {
          return _callback(
            'The source file ' + sourceFile.name + ' is too large'
          )
        }
    ...
    ctx.progressCallback = function (partNumber, loaded) {
      ...
      progressCallback(
        ctx.finishedBytes,
        ctx.uploadCheckpoint.fileStat.fileSize,
        (new Date().getTime() - ctx.start) / 1000
      )
    }
    ...
    if (!uploadCheckpoint.uploadId) {
      ...
      if (!contentType && uploadCheckpoint.key) {
        contentType =
          that.util.mimeTypes[
            uploadCheckpoint.key.substring(
              uploadCheckpoint.key.lastIndexOf('.') + 1
            )
          ]
      }
      ...
      if (!contentType && uploadCheckpoint.sourceFile.name) {
        contentType =
          that.util.mimeTypes[
            uploadCheckpoint.sourceFile.name.substring(
              uploadCheckpoint.sourceFile.name.lastIndexOf('.') + 1
            )
          ]
      }
