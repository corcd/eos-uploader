...
var checkPrefix = function (prefix) {
  if (
    Object.prototype.toString.call(prefix) === '[object String]' &&
  ...
  ObsClient.prototype.dropFolder = function (param, callback) {
    ...
    var recursiveDropByFolder = function (ctx, bucket, prefix, marker, done) {
      ...
      that.listObjects(
        ...
        function (err, result) {
          ...
          for (
            let j = 0;
            j < result.InterfaceResult.CommonPrefixes.length;
            j++
          ) {
            let subFolder = checkPrefix(
              result.InterfaceResult.CommonPrefixes[j]['Prefix']
            )
            if (runningTask < taskNum) {
              recursiveDropByFolder(
                {
          ...
          for (let j = 0; j < result.InterfaceResult.Contents.length; j++) {
            ...
            doDropOne(
              ...
              key.lastIndexOf(delimiter) === key.length - 1
            )
          }
    ...
    recursiveDropByFolder(
      ...
      function (ctx) {
        ...
        let errMessage =
          'drop folder ' + folder + ' failed due to child file deletion failed'
